...
public class PersonProfileManagerImpl implements PersonProfileManager {
    ...
    public PersonProfileDTO update(PersonProfileDTO majProfile) {
        Optional<PersonProfile> d = personProfileDAO.findById(majProfile.getId());
        PersonProfile t = personProfileDAO.saveAndFlush(orikaBeanMapper.map(d , PersonProfile.class));
        return orikaBeanMapper.map(t, PersonProfileDTO.class);
    }
}
